...
const RewindScreen1 = ({ pageNumber, totalPages, isActive }) => {
  ...
  bg-clip-text text-transparent mb-4 animate-slide-from-top"
          ...
          <span key={`maria-${animationKey}`} className="font-golos text-[32px] leading-[32px] font-bold animate-zoom-in-delayed">
            "María Camila"
          </span>
          <div key={`anos-${animationKey}`} className="flex items-center mt-4 animate-slide-from-left-delayed-1">
            <span className="text-[30px] leading-[36px] font-golos">
              10 años de
            </span>
            <img src={rappiLogo} alt="" className="" />
          </div>
          <span key={`huella-${animationKey}`} className="text-[30px] font-medium font-golos leading-[36px] animate-slide-from-left-delayed-2">
